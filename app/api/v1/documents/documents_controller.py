...
async def uploadDocument(projectId: int = Form(...), file: UploadFile = Form(...), categoryId: int = Form(...), metadata: str = Form(...), currentUser: Dict = Depends(get_current_user)):
    document = await DocumentService.handleUploadDocument(file, projectId, categoryId, metadata, currentUser)
    return document
...
async def uploadDocumentOllama(projectId: int = Form(...), file: UploadFile = Form(...), categoryId: int = Form(...), metadata: str = Form(...), currentUser: Dict = Depends(get_current_user)):
    document = await DocumentService.handleUploadDocumentByollama(file, projectId, categoryId, metadata, currentUser)
    return document
...
async def listDocuments(categoryId: int, currentUser: Dict = Depends(get_current_user)):
    return await DocumentService.handleListAllDocumentsByCategoryId(categoryId, currentUser)
